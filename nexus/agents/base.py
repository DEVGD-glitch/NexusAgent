...
class BaseAgent(ABC):
        ...
        **kwargs,
        ...
        try:
            ...
            if fallback_agent_type:
                ...
                # The registry should handle spawning the right agent type
                # For now, return the error result with fallback suggestion
                return AgentResult(
                    agent_id=self._context.agent_id if self._context else "unknown",
                    agent_type=self.agent_type,
                    status=AgentStatus.FAILED,
                    error=f"Primary failed: {e}. Suggested fallback: {fallback_agent_type}",
                    tools_used=self._tools_used,
                    started_at=datetime.now(timezone.utc).isoformat(),
                    completed_at=datetime.now(timezone.utc).isoformat(),
                    token_usage=self._token_usage,
    ...
    async def _call_llm(
        ...
        provider: Optional[str] = "gemini",  # Default to Gemma for function calling
        **kwargs,
    ) -> str:
        """
