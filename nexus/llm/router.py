...
class LLMRouter:
    ...
    async def _call_glm_direct(
        ...
        if response.status_code == 429:
            raise LLMRateLimitError(provider="glm")
        if response.status_code != 200:
            raise LLMProviderError(
                provider="glm",
    ...
    async def _call_ollama_direct(
        ...
        """Call Ollama local model directly."""
        ...
        # Ollama doesn't support tools in direct calls, return empty tool_calls
