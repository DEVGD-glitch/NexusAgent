// ═══════════════════════════════════════════════════════════════
// NEXUS — Avatar (SVG minimal with expressions)
// ═══════════════════════════════════════════════════════════════
...
export function NexusAvatar({ expression, thinking, size = 40, className = "" }: AvatarProps) {
  const mouthD = MOUTH_PATHS[expression] || MOUTH_PATHS.neutral;
  const eyeY = expression === "joy" ? 40 : 42;
  const uid = `av-${Math.random().toString(36).slice(2, 6)}`;

  return (
    <motion.div
      className={`relative ${className}`}
      animate={thinking ? { scale: [1, 1.04, 1] } : { scale: 1 }}
    ...
    >
      <svg viewBox="0 0 100 100" width={size} height={size} className="drop-shadow-lg">
        <defs>
          <radialGradient id={`g-${uid}`} cx="50%" cy="35%" r="55%">
        ...
        <circle cx="50" cy="50" r="52" fill={`url(#gl-${uid})`} />
        <circle cx="50" cy="50" r="45" fill={`url(#g-${uid})`} opacity={thinking ? 0.7 : 0.9} />
        <circle cx="50" cy="50" r="45" fill="none" stroke="rgba(255,255,255,0.08)" strokeWidth="1" />
        {thinking && (
          <>
            <circle cx="50" cy="50" r="48" fill="none" stroke="rgba(94,234,212,0.25)" strokeWidth="1.5" strokeDasharray="6 4">
              <animateTransform attributeName="transform" type="rotate" from="0 50 50" to="360 50 50" dur="4s" repeatCount="indefinite" />
            </circle>
            <circle cx="50" cy="50" r="51" fill="none" stroke="rgba(6,182,212,0.12)" strokeWidth="1" strokeDasharray="3 6">
        ...
        <path d={mouthD} stroke="white" strokeWidth={expression === "joy" ? 2.8 : 2.2} fill="none" strokeLinecap="round" />
      </svg>
      <div className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-background ${
        thinking ? "bg-emerald-500 shadow-[0_0_6px_rgba(16,185,129,0.5)]" : "bg-muted-foreground/30"
      }`} />
    </motion.div>
  );
}
