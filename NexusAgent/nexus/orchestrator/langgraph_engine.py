...
async def run_nexus_task(
    ...
    result = await _run_simple_loop(task, messages or [])

    # NOTE: LangGraph-based execution is available but currently disabled due to
    # state update issues. To re-enable, uncomment the code below and remove the
    # simple loop call above.
    #
    ...
    #     if graph is None:
    #         raise OrchestratorError("LangGraph not available")
