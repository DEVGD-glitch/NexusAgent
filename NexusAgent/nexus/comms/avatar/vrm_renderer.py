    ...
    async def initialize(self, vrm_path: Optional[str] = None) -> None:
        ...
        try:
            ...
            async def _ws_handler(websocket):
                ...
                try:
                    async for message in websocket:
                        pass  # Client messages are not needed
            ...
            self._ws_server = await websockets.serve(
                _ws_handler, "localhost", 18080
            )
            logger.info("[VRM] WebSocket server started on ws://localhost:18080")
        except ImportError:
            logger.warning("[VRM] websockets package not available, WebSocket server not started")
        except Exception as e:
            logger.warning("[VRM] WebSocket server failed to start: %s", e)
