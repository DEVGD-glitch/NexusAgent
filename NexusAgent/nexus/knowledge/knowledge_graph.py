...
class KnowledgeGraph:
    ...
    def remove_entity(self, name: str) -> bool:
        ...
        if not node_id:
            return False
        # Clean up _rel_type_index for all edges involving this node
        edges_to_remove = []
        for rel_type, edge_list in list(self._rel_type_index.items()):
            filtered = [(s, t) for s, t in edge_list if s != node_id and t != node_id]
            if filtered:
                self._rel_type_index[rel_type] = filtered
            else:
                del self._rel_type_index[rel_type]
