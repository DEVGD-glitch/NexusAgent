...
class NexusMemoryService:
    ...
    async def update(
        ...
        if text is not None:
            # When text is updated, merge new metadata with existing metadata
            ...
            merged_meta["updated_at"] = datetime.now(timezone.utc).isoformat()
            merged_meta["doc_hash"] = self._compute_hash(text)
            update_params["documents"] = [text]
            update_params["metadatas"] = [merged_meta]
