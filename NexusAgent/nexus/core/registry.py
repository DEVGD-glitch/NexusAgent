...
class AgentRegistry:
    ...
    def spawn(
        ...
        **kwargs,
        ...
        # Invoke the factory to create the actual agent if available
        factory = self._agent_types[agent_type].get("factory")
        if factory is not None:
            try:
                agent_obj = factory(task=task, **kwargs)
                instance.status = AgentStatus.RUNNING
                instance.started_at = datetime.now(timezone.utc).isoformat()
                # Store reference to the actual agent object for later use
                self._instances[instance.instance_id] = instance
                logger.info("Spawned agent %s of type '%s' via factory for task: %s",
                             instance.instance_id, agent_type, task[:100])
            except Exception as e:
                logger.error("Factory for agent type '%s' failed: %s", agent_type, e)
                instance.status = AgentStatus.FAILED
                instance.error = str(e)
        else:
            logger.info("Spawned agent %s of type '%s' for task: %s (no factory)",
                         instance.instance_id, agent_type, task[:100])
