        ...
        **kwargs,
        ...
        try:
            ...
            for dest_type in NEXUS_AGENT_DEFINITIONS[start_agent]["handoff_destinations"]:
                ...
                # Add handoff tool to the start agent using SDK handoff() helper
                try:
                    handoff_tool = handoff(target)
                    if hasattr(start_sdk_agent, 'handoffs'):
                        start_sdk_agent.handoffs.append(handoff_tool)
                    else:
                        start_sdk_agent.handoffs = [handoff_tool]
                except Exception as he:
                    logger.warning("Could not configure handoff to %s: %s", dest_type.value, he)
